...
2. **Stuck-transaction recovery** (every 5 min): find `status IN ('DISPATCHED','PROCESSING','RETRY') AND updated_at < NOW() - INTERVAL '10 minutes'`. Under `max-retries`: walk the legal state-machine path `→ FAILED → RETRY → DISPATCHED` (ADR-005 — no teleporting; the stuck timeout is recorded as the failure reason) and re-dispatch. At the cap: dead-letter. `RETRY` is in the query so a re-dispatch whose republish was lost (Kafka down during recovery) is swept up on a later cycle; a consumer's own mid-backoff `RETRY` is far younger than the timeout and is never mistaken for stuck.
